...
public final class KokoroTTS {
  ...
  // MoxSpeak patch: expose the phoneme string. G2P is the part of this pipeline most
  // likely to be wrong and a wrong pronunciation is invisible in every acoustic metric,
  // so callers need to be able to read the phonemes without running synthesis.
  // See Sources/Vendor/VENDORED.md.
  public func phonemize(text: String, language: Language) throws -> String {
    try updateLanguageIfNeeded(language)
    return try phonemizeText(text).0
  }
